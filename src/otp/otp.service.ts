...
export class OtpService {
  ...
  //   // OTP creation date and time
  //     const otpCreationDate = moment.tz(otp.created_at, 'Asia/Kathmandu').toDate();
  //     let diff= Math.floor((currentDate.getTime() - otpCreationDate.getTime()) / 1000);
  //     // let diff = getSecondsDifference( new Date(date),new Date(otp.created_at.toLocaleString()));  
  //     console.log(diff, 'daetime')
  //     if(diff>120){
  //       console.log('ljsdfs')
  //     throw new HttpException("Otp is only valid for 2 min",HttpStatus.BAD_REQUEST);
  //     }else{
